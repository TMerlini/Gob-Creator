...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async async function loadGallery(layerType) {
        ...
        try {
            ...
            if (data.images && data.images.length > 0) {
                ...
                // Enable drag and drop reordering for layer1 and layer2 only
                if (layerType === 'layer1' || layerType === 'layer2') {
                    initDragAndDropForGallery(gridElement, layerType);
                    
                    // Add save order button if it doesn't exist
                    if (!galleryElement.querySelector('.save-order-btn')) {
                        const saveOrderBtn = document.createElement('button');
                        saveOrderBtn.className = 'primary-btn save-order-btn';
                        saveOrderBtn.textContent = 'Save Image Order';
                        saveOrderBtn.addEventListener('click', () => saveImageOrder(layerType));
                        galleryElement.appendChild(saveOrderBtn);
                    }
                }
            } else {
    ...
    function initDragAndDropForGallery(galleryGrid, layerType) {
        // Add drag and drop capabilities to gallery items
        ...
        // Add a position number to each item
        updateItemPositions(galleryGrid);
    }
    ...
    function handleDragStart(e) {
        ...
        const img = this.querySelector('img, audio');
        if (img) {
            e.dataTransfer.setDragImage(img, 20, 20);
        }
        
        e.dataTransfer.effectAllowed = 'move';
        e.dataTransfer.setData('text/plain', this.dataset.layerType);
    }
    ...
    function handleDrop(e) {
        e.stopPropagation(); // Stops redirects
        
        // Don't do anything if dropping on the same item
        if (draggedItem !== this) {
            const layerType = draggedItem.dataset.layerType;
    ...
    function handleDragEnd(e) {
        ...
        // Update all items to remove drag-over class
    ...
    function updateItemPositions(galleryGrid) {
        const items = galleryGrid.querySelectorAll('.gallery-item');
        
        items.forEach((item, index) => {
            // Add or update position indicator
            let positionEl = item.querySelector('.position-indicator');
            
            if (!positionEl) {
                positionEl = document.createElement('div');
                positionEl.className = 'position-indicator';
                item.appendChild(positionEl);
            }
            
            positionEl.textContent = index + 1;
            
            // Add position to dataset for saving
    ...
    async function saveImageOrder(layerType) {
        try {
            let galleryGrid;
            if (layerType === 'layer1') {
                galleryGrid = layer1Gallery.querySelector('.gallery-grid');
            } else if (layerType === 'layer2') {
                galleryGrid = layer2Gallery.querySelector('.gallery-grid');
            } else {
                return;
            }
            ...
            // Extract image filenames and their positions
            galleryItems.forEach(item => {
                let filename;
                if (layerType === 'music') {
                    // For music, get the filename from the text content
                    const filenameEl = item.querySelector('.gallery-filename');
                    if (filenameEl) {
                        filename = filenameEl.textContent;
                    }
                } else {
                    // For images, get from the img src
            ...
            if (orderedImages.length > 0) {
                const saveBtn = layerType === 'layer1' ? 
                    layer1Gallery.querySelector('.save-order-btn') : 
                    layer2Gallery.querySelector('.save-order-btn');
                ...
                if (saveBtn) {
                    ...
                    // Send the ordered images to the server
