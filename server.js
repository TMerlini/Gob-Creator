...
app.post('/api/reorder-images', (req, res) => {
    try {
        const { layer, images } = req.body;
        
        if (!layer || !images || !Array.isArray(images)) {
        ...
        // Determine directory path based on layer
        let dir;
        if (layer === 'music') {
            dir = path.join(__dirname, 'music');
        } else {
        ...
        if (!fs.existsSync(dir)) {
            return res.status(404).json({ error: 'Layer directory not found' });
        }
        
        // Get existing files to verify all requested images exist
        const existingFiles = fs.readdirSync(dir);
        const missingFiles = images.filter(img => !existingFiles.includes(img));
        
        if (missingFiles.length > 0) {
            return res.status(400).json({ 
                error: 'Some images in the requested order don\'t exist',
                missingFiles
            });
        }
        
        // Create a temporary directory for processing
        const tempDir = path.join(dir, '_temp_reorder');
        ...
        images.forEach((image, index) => {
            ...
            const filePrefix = layer === 'music' ? 'track' : 'image';
            const newFilename = `${filePrefix}${index + 1}${ext}`;
            
            // Copy file to temp directory
            fs.copyFileSync(path.join(dir, image), path.join(tempDir, newFilename));
        });
        ...
        const renamedFiles = fs.readdirSync(tempDir);
